...
static int lua_clrNOD(lua_State *L) {
...
static int lua_getNOD(lua_State *L) {
	int argc = lua_gettop(L);
	
	lua_pushinteger(L,numberOfDirectories);
	return 1;
}
...
static int lua_dirNoNOD(lua_State *L)
{
	int argc = lua_gettop(L);
	if (argc != 0 && argc != 1) return luaL_error(L, "Argument error: System.listDirectory([path]) takes zero or one argument.");
...
static const luaL_Reg System_functions[] = {
	...
	{"listDirectory_NoUpdate",     lua_dirNoNOD},
	...
	{"clearNumberOfDirectories",           	    lua_clrNOD},
	{"getNumberOfDirectories",           	    lua_getNOD},
	{0, 0}
};
