...
static int lua_playwav(lua_State *L) {
	...
	sound_playwav(luaL_checkstring(L, 1));
	return 1;
}



static int lua_playwav_csf(lua_State *L) {
	int argc = lua_gettop(L);
	if (argc != 2) return luaL_error(L, "loadADPCM takes only 1 argument");
	sound_playwav_csf(luaL_checkstring(L, 1), luaL_checkinteger(L, 2));
	return 1;
}


static int lua_playwav_csf_controls(lua_State *L) {
	int argc = lua_gettop(L);
	if (argc != 3) return luaL_error(L, "loadADPCM takes only 1 argument");
	sound_playwav_csf_controls(luaL_checkstring(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3));
	return 1;
}
...
static const luaL_Reg Sound_functions[] = {
	...
	{"playWav",      							 lua_playwav},
	{"playWav_CustomFrequency",      							 lua_playwav_csf},
	{"playWav_WithControls",      							 lua_playwav_csf_controls},
	{0, 0}
};
